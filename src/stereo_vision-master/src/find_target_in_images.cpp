...
const std::string face_cascade_name = "/home/xu/dogkin_ws/src/stereo_vision-master/haarcascade_frontalface_alt.xml";
...
//	cv::Point center(0, 0);
	...
	equalizeHist( grayImg, grayImg ); //直方图均衡化，拉伸像素使其分布到0-255,增加对比度
       int h_min = 160, s_min = 80, v_min = 50;//筛选出红色区域的值
       int h_max = 180, s_max = 255, v_max = 220;
       Scalar hsv_min = Scalar(h_min, s_min, v_min);
       Scalar hsv_max = Scalar(h_max, s_max, v_max);
       Mat dstImg = Mat::zeros(img.size(), CV_8U);//dstImg需同frame相同大小，且类型为CV_8U 
       inRange(grayImg, hsv_min, hsv_max, dstImg);//属于这个范围的会变成白色，其他为黑，存放在dstImg中 
  	 medianBlur(dstImg, dstImg, 5);//中值滤波去除噪声          
//blur(grayImg,grayImg,Size(7,7));
        //threshold(canny_output,thresholdFrame,segVlaue,255,CV_THRESH_BINARY);
        //faceCascade.detectMultiScale( grayImg, faces, 1.1, 2, 0|CASCADE_SCALE_IMAGE, Size(30, 30) ); //进行人脸检测
       // Canny( dstImg, canny_output, thresh, thresh*3, 3 );
        findContours(dstImg, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0));//寻找特征点
...
{        
faceRectangle.center.x = mc[i].x ;
        faceRectangle.center.y = mc[i].y;
        faceRectangle.size.width = mu[i].m10;
        faceRectangle.size.height =mu[i].m00;
        faceRectangle.angle = 0;
        return faceRectangle;
