...
void CalTargetPos(geometry_msgs::Point& leftPoint, geometry_msgs::Point& rightPoint)
{
  ...
  else //这部分公式需要推导(深度）
  {
     ...
     //target3DPosition.point.z =0;
	target3DPosition.point.z = DISTANCE_BETWEEN_CAMERAS*CAMERA_MODEL_FX/(leftPoint.y - rightPoint.y);
  }
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber left2DPositionSub = nh.subscribe("/edge_detector/left_2Dposition", 1, leftPointCallback); //订阅左图像话题
  ros::Subscriber right2DPositionSub = nh.subscribe("/edge_detector/right_2Dposition", 1, rightPointCallback); //订阅右图像话题
