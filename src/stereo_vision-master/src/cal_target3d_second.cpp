...
#define DISTANCE_BETWEEN_CAMERAS 0.062 //（单位：m）两摄像头之间的距离，标定得到
...
geometry_msgs::PointStamped target3DPosition_second; //在rviz中显示

void leftPointCallback(const geometry_msgs::Point& leftPoint_second) //形参到底怎么写？::::
{
...
{
  ros::init(argc, argv, "cal_target_3Dposition_second"); //节点名称：cal_target_3Dposition
  ros::NodeHandle nh; //创建与master通信的节点
  ros::Rate r(SUBSCRIBE_FREQUENCY); //执行频率5Hz
  target3DPositionPub_second = nh.advertise<geometry_msgs::PointStamped>("target_3Dposition_second", 1); //发布话题：target_3Dposition
  ros::Subscriber left2DPositionSub = nh.subscribe("/edge_detector/left_2Dposition_second", 1, leftPointCallback); //订阅左图像话题
  ros::Subscriber right2DPositionSub = nh.subscribe("/edge_detector/right_2Dposition_second", 1, rightPointCallback); //订阅右图像话题
